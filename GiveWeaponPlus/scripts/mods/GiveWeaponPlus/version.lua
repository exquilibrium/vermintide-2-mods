...
-- 2026-09-05 21:43 UTC
local OUR_VERSION_TIMESTAMP = 1788644624

-- The "last_build" checkbox's displayed label (see GiveWeaponPlus_data.lua -
-- it's a no-op checkbox, not read anywhere, just a way to surface this as a
-- label in the options list) comes straight from the "last_build" entry in
-- GiveWeaponPlus_localization.lua - "_Version Mod.bat" stamps that entry's
-- text at the same time it stamps OUR_VERSION_TIMESTAMP below. It's baked
-- in as static text rather than computed here at runtime because the mod
-- options menu reads widget labels directly from the localization table,
-- not through mod:localize(), so a runtime override here has no effect on it.
