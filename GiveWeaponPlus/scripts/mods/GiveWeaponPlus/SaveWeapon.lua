--[[
	...
	Originally a standalone mod, folded into GiveWeaponPlus so item creation and saving
	live under a single mod.
	...
	Summary:
	¯¯¯¯¯¯¯
	This mod saves items created with the weapon creator and loads them up next time you launch the game.
...
-- This file is dofile'd from GiveWeaponPlus.lua, so `mod` here is the same GiveWeaponPlus mod object
-- that create_weapon/the UI live on.
...
mod:dofile("scripts/mods/"..mod:get_name().."/SaveWeapon_utilities")

-- # One-time migration from the standalone SaveWeapon mod # --
-- VMF settings are namespaced per mod name, so now that SaveWeapon's functionality lives
-- inside GiveWeaponPlus, existing SaveWeapon users' saved items/keybinds/settings need to be
-- copied over once. This only reads SaveWeapon's old data (via the raw engine setting API,
-- since the SaveWeapon mod may no longer be registered) -- it never writes back to
-- SaveWeapon's slot, since VMF's own autosave would silently clobber a direct write to
-- another mod's settings.
if not mod:get("migrated_from_save_weapon") then
	local old_settings = (Application.user_setting("mods_settings") or {})["SaveWeapon"]
	if old_settings then
		...
		for _, key in ipairs(migrated_keys) do
			if old_settings[key] ~= nil then
				mod:set(key, old_settings[key])
			end
		end
		mod:echo("[GiveWeaponPlus] Migrated saved items and settings from the old SaveWeapon mod.")
	end
	mod:set("migrated_from_save_weapon", true)
end
...
-- There are two separate inventory-browsing screens in the game -- the per-slot loadout browser
-- (HeroWindowLoadoutInventory) and the general Inventory tab (HeroWindowInventory) -- each with
-- its own independent ItemGridUI. HeroWindowLoadoutInventory's on_enter/on_exit are NOT hooked
-- here: GiveWeaponPlus.lua already hooks both (to show/hide its own window) from the same mod
-- object, and VMF silently drops a second hook_safe on the same target from the same mod instead
-- of stacking it, so item_grid tracking for that screen lives there instead. Only need to add it
-- here for the screen GiveWeaponPlus.lua doesn't already hook.
mod:hook_safe(HeroWindowInventory, "on_enter", function(hero_window_inventory)
	mod.item_grid = hero_window_inventory._item_grid
end)
mod:hook_safe(HeroWindowInventory, "on_exit", function(hero_window_inventory)
	mod.item_grid = nil
end)
...
--[[
	...
	Maybe I should put a hard cap at 2 properties, but the weapon creator doesn't so for now I didn't bother. Theoretically you could have one of each property.
]]--
...
mod.create_saved_item = function(self, backend_id, savestring)
	...
	if mod:is_item_key_valid(item_key) then
		...
		mod:pcall(function()
			...
			-- This should only occur if you use the weapon creator's default skin option.
			if not skin then
				item.skin = nil
			end
		end)
...
mod:hook_safe(ItemHelper, "mark_backend_id_as_favorite", function(backend_id, save)
	-- Only act if the item comes from this mod
	if mod:is_backend_id_from_mod(backend_id) then
		local save_id = mod:get_backend_save_id(backend_id)
...
-- # Changes displayed rarity of all created items # --
...
-- ... and apply the updated rarity to all created items
