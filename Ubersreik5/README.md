...
A single `StateIngame.on_enter` hook resets everything - the mod's own `PlayerScores` table and vanilla's own native stats - whenever `game_mode_key()` is `"inn"`, `"inn_deus"`, or `"adventure"`.

`on_enter` alone is enough to cover every case that matters (win, loss, restart/reload, and the next mission after any of those), because all of them are genuine `StateIngame` exit/re-entry transitions, not in-place reuse - confirmed via `reload_level`'s own exit path (`state_ingame.lua`: `exit_type = "reload_level"` -> `return StateLoading` -> eventually a fresh `StateIngame.on_enter`). A win or a full loss (no checkpoint) both route through `AdventureMechanism.game_round_ended`'s `set_next_level(hub, ...)` back to the inn; a checkpoint-available wipe auto-starts the `continue_level` vote, whose `on_complete` calls `reload_level`; either way, the next `StateIngame.on_enter` is where the reset happens. An earlier version of this also hooked `reload_level` directly as "belt-and-suspenders" insurance against `on_enter` not re-firing in time for a restart specifically - that turned out to be unnecessary once the exit-path was actually traced, so it was removed to keep this to one hook.

**Scope is deliberately narrow**: `game_mode_key()` is `"adventure"` for every adventure mission level (not just the hub - see `level_transition_handler.lua`'s `gamemode_lookup.default`), which is what makes this also reset between two missions played back-to-back without returning to the keep, not just on inn entry. It does *not* cover `"weave"` (Winds of Magic weaves) or Chaos Wastes' own in-run states (`"ingame_deus"`/`"map_deus"`, distinct from the `"inn_deus"` hub-equivalent) - out of scope for what this mod targets. Note also that Chaos Wastes' actual end-of-run scoreboard doesn't read from `statistics_db` at all (`DeusMechanism.get_players_session_score` returns `self._deus_run_controller:get_scoreboard()` instead, a separate, intentionally-cumulative-across-legs tracker) - so a native-stat reset wouldn't even affect what's displayed there.

The reset itself does three things:
1. **`PlayerScores = {}`** - the mod's own custom stat table.
2. **`statistics_db:reset_session_stats()`** - vanilla's own native stats (kills/damage/etc). The `StatisticsDatabase` backing these (`Managers.venture.statistics`) is created once per "venture" (`GameMechanismManager._on_venture_start`) and only swapped for a fresh one when the venture ends (returning to inn) - so without an explicit reset, it silently accumulates across every mission played in one venture instead of resetting per mission. Vanilla itself defines `StatisticsDatabase:reset_session_stats()` for exactly this purpose (zeroes every registered player's session stat back to its default/persistent value in place, no unregister/re-register needed) but never calls it anywhere in real gameplay - only from its own unit test.
...
**What this deliberately doesn't reset**: a teammate's crash or forced disconnect mid-mission. That's handled by the separate `NativeStatsSnapshot`/`mod.on_user_joined`/`CATCHUP_PACKAGE_ID` mechanism below, whose whole point is to *preserve* that player's stats across the disconnect and restore them on rejoin - the opposite of resetting.
...
AI Director player-position clustering (`conflict_director/conflict_utils.lua`), hardcoded to at most 4 players/bots. Called constantly throughout a mission: `horde_spawner.lua` uses `cluster_positions` to decide *where* to spawn hordes relative to the party, `conflict_director.lua` uses both for pacing, and `perception_utils.lua` uses `cluster_weight_and_loneliness` to pick which player is most "lonely" (e.g. who an isolated-player-seeking special like a Gutter Runner should target). Without patching these, a 5th player's position is silently invisible to all of that - not a visual bug, a whole-mission gameplay-pacing one. Both hooks for `ConflictUtils` live here, together - keeping every conflict-director-related hook in one file instead of split across `Ubersreik5.lua`.

**Full replace rather than wrapping vanilla's function**, for both: vanilla's originals close over module-level scratch tables reused across calls as a performance optimization (avoids a table allocation per call), and those locals aren't reachable from a hook. `cluster_positions` mirrors that same reuse (`cluster_positions_sizes`/`cluster_positions_index_lookup`/`cluster_positions_work_queue`, module-level locals in this file), but clears each one by capturing its *own* live length from the previous call before wiping it, rather than vanilla's hardcoded `for i = 1, 3 do work_queue[i] = nil end` - vanilla's fixed reset size only covers up to 3 queued positions (correct for its own 4-player cap, one of which never enters the queue), so a 5th player's queue slot would go uncleared and a call with fewer positions right after a call with more would see stale leftover entries and miscount how many are actually queued. Capturing the length live avoids that regardless of player count. `cluster_weight_and_loneliness` doesn't have any scratch state to reuse (its `loneliness` result table is freshly allocated per call).

**Framework note:** an earlier version of this mod also had a `cluster_positions` hook duplicated in `Ubersreik5.lua` itself (left over from before this file existed). Two `mod:hook` registrations for the same `(table, method)` pair from the *same* mod don't chain in this framework - see the hook-chaining limitation above - so that duplicate was silently dead code (Ubersreik5.lua's own copy, registered first, was the one actually running; this file's was ignored). Consolidated here to have one canonical implementation and avoid hitting that limitation again.
