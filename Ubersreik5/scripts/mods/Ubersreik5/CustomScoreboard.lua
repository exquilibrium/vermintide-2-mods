...
-- A player whose profile hasn't finished syncing yet (still connecting/
-- loading - e.g. a weak connection right as the mission ends) makes
-- ProfileSynchronizer.profile_by_peer return nil, nil by design (it's not
-- in get_peers_with_full_profiles() yet). ScoreboardHelper.
-- get_grouped_topic_statistics never guards against that nil and does
-- SPProfiles[profile_index].careers[career_index] unconditionally while
-- building EVERY player's data in one loop - so one still-connecting player
-- hard-crashes the whole thing before any player's native topics or hero
-- portrait get built, not just that one player's. This is vanilla's own
-- gap (adventure mode has no get_players_session_score override, so this
-- is the only function that ever builds this data - see README.md), but a
-- 5th real player means more simultaneous connections at end-of-level and
-- so a higher chance one hasn't finished syncing in time. See README.md.
mod:hook(ScoreboardHelper, "get_grouped_topic_statistics", function (func, statistics_db, profile_synchronizer, saved_scoreboard_data)
	if not profile_synchronizer then
		return func(statistics_db, profile_synchronizer, saved_scoreboard_data)
	end

	local original_profile_by_peer = profile_synchronizer.profile_by_peer

	-- Instance-level override (not a class-level hook) - only affects calls
	-- made through this specific profile_synchronizer object, and only
	-- while this function runs (restored below).
	profile_synchronizer.profile_by_peer = function (self, peer_id, local_player_id)
		local profile_index, career_index = original_profile_by_peer(self, peer_id, local_player_id)
		...
		-- Not in get_peers_with_full_profiles() yet (still connecting), but
		-- the host already knows what they picked - try_reserve_profile_for_
		-- peer writes this same peer's persistent_hero_reservation as a side
		-- effect (NetworkState.set_profile_index_reservation) at lobby
		-- character-select time, well before the later full-profile RPC
		-- round-trip this player hasn't finished yet. Vanilla's own
		-- versus-mode equivalent (ScoreboardHelper.get_versus_stats) already
		-- falls back to this same persistent record for this same reason -
		-- get_grouped_topic_statistics (adventure mode) just never got the
		-- same treatment. 0/0 is the documented "never reserved anything"
		-- sentinel (network_state_spec.lua). See README.md.
		local persistent_profile_index, persistent_career_index = self:get_persistent_profile_index_reservation(peer_id)

		profile = persistent_profile_index ~= 0 and SPProfiles[persistent_profile_index]
		careers = profile and profile.careers

		if persistent_career_index ~= 0 and careers and careers[persistent_career_index] then
			return persistent_profile_index, persistent_career_index
		end

		-- Genuinely never reserved anything - generic portrait instead of a
		-- crash. Corrects itself once their profile actually finishes
		-- syncing.
		return 1, 1
	end

	local ok, result = pcall(func, statistics_db, profile_synchronizer, saved_scoreboard_data)

	profile_synchronizer.profile_by_peer = original_profile_by_peer

	if ok then
		return result
	end

	mod:warning("[Ubersreik5] get_grouped_topic_statistics still failed after the profile fallback: %s", tostring(result))
