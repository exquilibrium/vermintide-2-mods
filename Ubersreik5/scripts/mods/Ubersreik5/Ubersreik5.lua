...
-- See README.md.
--
-- Keeps vanilla's own idempotency check and conditional lobby-data-sync
-- flag (only the "someone else already holds this" block is removed, since
-- allowing that is the entire point of this stub) - a peer re-confirming a
-- pick they already hold (e.g. a retried/duplicate rpc_request_profile,
-- more likely over a weak connection) is a no-op instead of an
-- unconditional clear+rewrite every single call. See README.md.
mod:hook_origin(ProfileSynchronizer, "try_reserve_profile_for_peer", function (self, party_id, peer_id, profile_index, career_index)
	...
	if reserver_peer == peer_id and career_index == persistent_career_index then
		return true
	end
...
-- directly instead of trusting is_profile_in_use. See README.md.
mod:hook(GameModeAdventure, "_get_first_available_bot_profile", function (func, self)
	...
	-- Respect PlayerData.bot_spawn_priority (the user-configured bot hero
	-- priority order, read into self._bot_profile_id_to_priority_id by
	-- _setup_bot_spawn_priority_lookup) instead of picking randomly among
	-- what's free - matches vanilla's own sort-and-take-first, just over
	-- this mod's real-usage-filtered list instead of vanilla's
	-- is_profile_in_use-filtered one. See README.md.
	...
	if #free_profile_indices > 0 then
		table.sort(free_profile_indices, by_priority)

		profile_index = free_profile_indices[1]
	else
		local all_profile_indices = {}

		for i = 1, #available_profiles do
			all_profile_indices[i] = FindProfileIndex(available_profiles[i])
		end

		table.sort(all_profile_indices, by_priority)

		profile_index = all_profile_indices[1]
	end
